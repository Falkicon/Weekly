...
describe("FenUI Core", function()
	...
	describe("FenUI Base", function()
		it("should handle theme changed callbacks", function()
			local called = false
			local id = FenUI:OnThemeChanged(function()
				called = true
			end)
			...
			local id2 = FenUI:OnThemeChanged(function()
				error("fail")
			end)
		...
		it("should provide SafeCall", function()
			local success, res = FenUI.SafeCall(function(a)
				return a
			end, "test")
			assert.is_true(success)
			assert.is_equal(res, "test")

			local success2 = FenUI.SafeCall(function()
				error("fail")
			end)
