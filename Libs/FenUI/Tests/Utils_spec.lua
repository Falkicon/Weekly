...
describe("FenUI Utils", function()
	...
	describe("SafeAPI", function()
		it("should call functions safely", function()
			local called = false
			local success, results = FenUI.Utils:SafeCall(function()
				called = true
				return "ok"
			end)
		...
		it("should handle unit health percent (Midnight)", function()
			...
			_G.UnitHealthPercent = function(u)
				return 75
			end
		...
		it("should get action texture safely", function()
			...
			-- Test table return
			_G.C_ActionBar.GetActionTexture = function()
				return { icon = "test_icon" }
			end
		...
		it("should handle action texture safely (with ID)", function()
			...
			_G.C_ActionBar.GetActionTexture = function(id)
				if id == 999 then
		...
		it("should get aura duration safely", function()
			...
			_G.C_UnitAuras.GetAuraDurationRemaining = function()
				return 5
			end
		...
		it("should handle SetCooldown safely", function()
			...
			cd.SetCooldown = function(self, s, d)
				self.s = s
				self.d = d
			end
			...
			cd.SetCooldownFromDurationObject = function(self, obj)
				self.obj = obj
			end
			local durObj = {
	...
	describe("UI", function()
		...
		it("should handle parent resize hooks", function()
			...
			local called = false
			child.UpdateDynamicSize = function()
				called = true
			end
		...
		it("should handle intrinsic size observation", function()
			...
			local child = CreateFrame("Frame")
			local called = false
			parent.UpdateDynamicSize = function()
				called = true
			end
		...
		it("should handle GetOrCreateWidget", function()
			local parent = {}
			local widget = FenUI.Utils:GetOrCreateWidget(parent, "myWidget", function()
				return { name = "new" }
			end)
			assert.is_equal("new", widget.name)
			local existing = FenUI.Utils:GetOrCreateWidget(parent, "myWidget", function()
				return { name = "other" }
			end)
