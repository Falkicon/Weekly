...
local function SaveJournalData()
    if not Journal.tracker then return end
...
local function LoadJournalData()
    if not Journal.tracker then return false end
    if not ns.Config.journal then return false end
    if not ns.Config.journal.categories then return false end
...
local function OnAchievementEarned(tracker, event, achievementID, alreadyEarned)
    if alreadyEarned then return end  -- Skip if this was already earned before
    
    local id, name, points, completed, month, day, year, description, 
          flags, icon, rewardText, isGuild, wasEarnedByMe, earnedBy, isStatistic = GetAchievementInfo(achievementID)
...
local function OnNewMountAdded(tracker, event, mountID)
    ...
    local name, spellID, icon, isActive, isUsable, sourceType, isFavorite, 
          isFactionSpecific, faction, shouldHideOnChar, isCollected, mountID2, 
          isForDragonriding = C_MountJournal.GetMountInfoByID(mountID)
...
local function OnNewPetAdded(tracker, event, battlePetGUID)
    if not battlePetGUID then return end
    ...
    local speciesName, speciesIcon, petType, companionID, tooltipSource, 
          ...
          obtainable, creatureDisplayID = C_PetJournal.GetPetInfoBySpeciesID(speciesID)
...
local function OnNewToyAdded(tracker, event, itemID, isNew)
    ...
    if not isNew then return end  -- Only track newly added toys
...
local function IsGatheringMaterial(itemID)
    local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, 
          ...
          classID, subclassID, bindType, expansionID = C_Item.GetItemInfo(itemID)
...
function Journal:Initialize()
    if self.tracker then return end  -- Already initialized
    ...
    if itemCount > 0 or gatheringCount > 0 then
        ns.Weekly:Printf("Journal loaded: %d collectibles, %d materials gathered", itemCount, gatheringCount)
    end
end
...
function Journal:OnItemLogged(category, id, data)
    ...
    if ns.Config.journal and ns.Config.journal.showNotifications then
        ...
        ns.Weekly:Printf("|cff00ff00[Journal]|r New %s: %s", categoryName, data.name or "Unknown")
    end
...
function Journal:OnGatheringLogged(itemID, quantity, data)
    ...
    if ns.Config.journal and ns.Config.journal.showNotifications and data.count == quantity then
        ns.Weekly:Printf("|cff00ff00[Journal]|r Started gathering: %s", data.name or "Unknown")
    end
...
function Journal:GetTotalCount()
    if not self.tracker then return 0 end
...
function Journal:GetGatheringTotalCount()
    if not self.gathering then return 0 end
