...
function JournalUI:CreateWindow()
    ...
    if not frame.useFenUI then
        ...
        frame.title:SetText("Weekly Journal")
    ...
    if FenUI and FenUI.CreateLayout then
        ...
        frame.clearTabBtn:SetText("Clear Tab")
        frame.clearTabBtn:SetScript("OnClick", function() self:OnClearTabClicked() end)
        ...
        frame.clearAllBtn:SetText("Clear All")
        frame.clearAllBtn:SetScript("OnClick", function() self:OnClearAllClicked() end)
            ...
            onRowBind = function(row, item, index)
                ...
                row.tooltipFunc = function()
                    ...
                    if item.data.zoneName then
                        GameTooltip:AddLine("Found in: " .. item.data.zoneName, unpack(C_GRAY))
                    end
                    GameTooltip:AddLine(" ")
                    GameTooltip:AddLine("Click to view in Collections", 0.5, 0.5, 0.5)
                    GameTooltip:Show()
                end
    ...
    if FenUI and FenUI.CreateEmptyState then
        self.emptyState = FenUI:CreateEmptyState(frame.contentContainer, {
            ...
            subtitle = "Items will appear here as you collect them this week",
            gap = 20,
        })
...
function JournalUI:CreateTabs()
    ...
    local tabDefs = {
        { key = "dashboard", text = "Dashboard", icon = "Interface\\Icons\\INV_Misc_Book_09" },
    }
...
function JournalUI:ClearContent()
    ...
    if self.itemGrid then self.itemGrid:Hide() end
...
function JournalUI:UpdateWeekLabel()
    ...
    self.frame.weekLabel:SetText(string.format("Week of %s %d", monthName, math.floor((date.monthDay - 1) / 7) * 7 + 1))
end
...
function JournalUI:RenderDashboard()
    ...
    headerRow.name:SetText("This Week's Collection")
    ...
    totalRow.name:SetText("Total Items")
    ...
    pointsRow.name:SetText("Achievement Points")
    ...
    dividerRow.name:SetText("By Category")
    ...
    for _, cat in ipairs(categories) do
        ...
        catRow.tooltipFunc = function(row)
            ...
            GameTooltip:AddLine("Click to view " .. cat.name, unpack(C_GRAY))
            GameTooltip:Show()
        end
        catRow:Show()
    ...
    gatheringDivider.name:SetText("Gathering")
    ...
    gatheringRow.name:SetText("Materials Gathered")
    gatheringRow.name:SetFontObject("GameFontNormal")
    gatheringRow.name:SetTextColor(unpack(C_WHITE))
    gatheringRow.extra:SetText(string.format("%d (%d types)", gatheringTotal, gatheringUnique))
    ...
    gatheringRow.tooltipFunc = function(row)
        GameTooltip:SetText("Gathering", unpack(C_GOLD))
        GameTooltip:AddLine(string.format("%d materials across %d types", gatheringTotal, gatheringUnique), unpack(C_WHITE))
        GameTooltip:AddLine("Click to view Gathering", unpack(C_GRAY))
        GameTooltip:Show()
    end
    gatheringRow:Show()
...
function JournalUI:RenderCategoryTab(category)
    ...
    if #items == 0 then
        ...
        if self.emptyState then
            ...
            self.emptyState:SetTitle("No items collected")
            self.emptyState:SetSubtitle("Items will appear here as you collect them this week")
            ...
            emptyRow.name:SetText("No items collected this week")
...
function JournalUI:RenderGatheringTab()
    ...
    if #expansions == 0 then
        -- Use FenUI EmptyState if available
        if self.emptyState then
            self.emptyState:SetTitle("No materials gathered")
            self.emptyState:SetSubtitle("Items will appear here as you gather them this week")
            self.emptyState:Show()
        else
            -- Fallback
            local emptyRow = self:GetRow(1)
            ...
            emptyRow.icon:Hide()
            emptyRow.name:SetPoint("LEFT", 4, 0)
            emptyRow.name:SetText("No materials gathered this week")
            emptyRow.name:SetFontObject("GameFontNormal")
            emptyRow.name:SetTextColor(unpack(C_GRAY))
            emptyRow.extra:SetText("")
            emptyRow.tooltipFunc = nil
    ...
    for _, expData in ipairs(expansions) do
        ...
        headerRow.name:SetFontObject("GameFontNormalLarge")
        headerRow.name:SetTextColor(unpack(C_GOLD))
        headerRow.extra:SetText(string.format("%d items", expData.totalCount))
        ...
        for _, item in ipairs(items) do
            ...
            -- Name (indented)
            row.name:SetPoint("LEFT", row.icon, "RIGHT", 6, 0)
            row.name:SetText(item.name or ("Item " .. item.itemID))
            row.name:SetFontObject("GameFontNormal")
            row.name:SetTextColor(unpack(C_WHITE))
            
            -- Count
            row.extra:SetText("x " .. item.count)
...
function JournalUI:OnClearTabClicked()
    if self.currentTab == "dashboard" then return end
    
    -- Confirmation dialog
    StaticPopupDialogs["WEEKLY_JOURNAL_CLEAR_TAB"] = {
        text = "Clear all %s from this week's journal?",
        button1 = "Clear",
        button2 = "Cancel",
        OnAccept = function()
...
function JournalUI:OnClearAllClicked()
    StaticPopupDialogs["WEEKLY_JOURNAL_CLEAR_ALL"] = {
        text = "Clear ALL items from this week's journal?",
        button1 = "Clear All",
        button2 = "Cancel",
        OnAccept = function()
            ns.Journal:ClearAll()
...
function JournalUI:SavePosition()
    if not self.frame then return end
    if not ns.Config.journal then return end
