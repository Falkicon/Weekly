...
-- Only load in dev mode (Discovery) or if Weekly Journal is enabled (future)
-- For now, this is dev-only infrastructure
...
function TrackerCore:LogItem(category, id, data)
    if not category or not id then return false end
...
function TrackerCore:AddButton(text, onClick)
    if not self.frame or not self.frame.buttonBar then return nil end
