...
if not ns.IS_DEV_MODE then return end
...
local function GetSavedData()
    if not WeeklyDB then return nil end
    if not WeeklyDB.dev then WeeklyDB.dev = {} end
...
local function LoadDiscoveryData(tracker)
    ...
    if not saved or not saved.items then return false end
...
local function PruneOldestItems(tracker, maxItems)
    if tracker.itemCount <= maxItems then return 0 end
...
function Discovery:Initialize()
    ...
    if loaded then
        ...
        if pruned > 0 then
            print(string.format("|cffff8800[Weekly Discovery]|r Pruned %d old items (cap: %d)", pruned, MAX_STORED_ITEMS))
        end
        print(string.format("|cff00ff00[Weekly Discovery]|r Loaded %d saved items (last saved: %s)", 
            self.tracker.itemCount, lastSaved or "unknown"))
    end
    ...
    self.tracker:RegisterEvents({
        -- Currency events
        CURRENCY_DISPLAY_UPDATE = function(t, event, currencyType, quantity, quantityChange, quantityGainSource, quantityLostSource)
...
function Discovery:OnItemLogged(category, id, data)
    ...
    print(string.format("|cff00ff00[Discovery]|r %s: %s (%d)%s%s%s",
        category, data.name or "Unknown", id, expansionTag, zoneTag, capTag))
end
...
function Discovery:UpdateRow(row, item)
    ...
    if data.expansionName then
        ...
        if abbrev == "The War Within" then abbrev = "TWW"
        elseif abbrev == "Dragonflight" then abbrev = "DF"
        elseif abbrev == "Shadowlands" then abbrev = "SL"
        elseif abbrev == "Battle for Azeroth" then abbrev = "BfA"
        elseif abbrev == "Midnight" then abbrev = "MN"
        end
