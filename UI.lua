...
function UI:Initialize()
    ...
    if not self.frame then
        ...
        self.frame:SetScript("OnEvent", function() self:RefreshRows() end)
...
function UI:EnforceAnchor()
    ...
    if not left then return end -- Frame not valid yet
...
function Utils.GetQuest(id)
    ...
    if id == 0 then return false, 0, 0, false, false, nil end 
...
function UI:RenderRows()
    ...
    for _, section in ipairs(sections) do
        ...
        for _, item in ipairs(section.items) do
             ...
             if type(checkID) == "table" then checkID = checkID[1] end
        ...
        if #items == 0 then
            ...
            for _, item in ipairs(items) do
                 ...
                 elseif item.type == "currency" or item.type == "currency_cap" then
                     ...
                     local valText = amt
                     if max > 0 then valText = amt .. " / " .. max end
                     self.measureFS:SetText(valText)
                     valueWidth = self.measureFS:GetStringWidth()
                 end
                 
                 if textWidth > maxLabelWidth then maxLabelWidth = textWidth end
                 if valueWidth > maxValueWidth then maxValueWidth = valueWidth end
    ...
    row.iconBtn:SetScript("OnEnter", function(self)
        ...
        elseif self.type == "vault" and self.details then
            ...
            if self.details and self.details.history and #self.details.history > 0 then
                ...
                if self.id == 3 then header = "Bosses Defeated:" end -- Raid
                ...
                for _, run in ipairs(self.details.history) do
                    ...
                    if not run.completed then rightText = rightText .. " (Failed)" end
...
function UI:UpdateRow(row, data, ctx)
    ...
    elseif data.type == "vault_visual" then
        ...
        if data.id == 3 then iconTexture = "Interface\\Icons\\Achievement_Boss_Ragnaros" end -- Raid
        if data.id == 1 then iconTexture = 525134 end -- Dungeon (ChallengeMode-Keystone-Empty)
        if data.id == 6 then iconTexture = "Interface\\Icons\\Achievement_WorldEvent_Lunar" end -- World
