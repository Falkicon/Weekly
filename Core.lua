...
function Weekly:OnInitialize()
    self:Printf("Loaded. Type /weekly to open.")
    ...
    if AddonCompartmentFrame and AddonCompartmentFrame.RegisterAddon then
        AddonCompartmentFrame:RegisterAddon({
            ...
            funcOnEnter = function(menuButtonFrame)
                ...
                GameTooltip:AddLine("Left-click: Toggle tracker", 0.7, 0.7, 0.7)
                GameTooltip:AddLine("Right-click: Open Journal", 0.7, 0.7, 0.7)
                GameTooltip:Show()
            end,
...
function Weekly:SlashHandler(msg)
    ...
    if cmd == "help" or cmd == "?" then
        ...
        self:Printf("  /weekly - Toggle weekly tracker window")
        self:Printf("  /weekly journal - Toggle journal window")
        self:Printf("  /weekly settings - Open settings")
        self:Printf("  /weekly help - Show this help")
        return
    end
    ...
    if cmd == "journal" or cmd == "j" then
        if ns.JournalUI then
            ...
            self:Printf("Journal is not available. Check settings to enable it.")
        end
        return
    end
    ...
    if cmd == "discovery" or cmd == "disc" then
        if ns.IS_DEV_MODE and ns.Discovery then
            ...
            self:Printf("Discovery tool is only available in dev mode")
        end
        return
    end
    ...
    if cmd == "debug" then
        ...
        self:Printf("Debug Mode: " .. (ns.Config.debug and "|cff00ff00ON|r" or "|cffff0000OFF|r"))
        ...
        if acts then
             ...
             for i, act in ipairs(acts) do
                 self:Printf(format("Slot %d: Tier %s, Level %s, Progress %s/%s", i, tostring(act.activityTierID), tostring(act.level), tostring(act.progress), tostring(act.threshold)))
                 if act.activityTierID then tierID = act.activityTierID end
             end
        else
        ...
        for i = 1, num do
            local name, id, reset, diff, locked, extended, instanceIDMostSig, isRaid, maxPlayers, diffName, numEncounters, encounterProgress = GetSavedInstanceInfo(i)
            if isRaid then
...
function Weekly:QUEST_TURNED_IN(event, questID, xp, money)
    if ns.Config.debug then
        self:Printf("Quest Completed: ID " .. questID)
    end
end

function Weekly:QUEST_ACCEPTED(event, questID)
    if ns.Config.debug then
        self:Printf("Quest Accepted: ID " .. questID)
    end
end
