...
function ConfigUI:Initialize()
	...
	local devOptions = {
		...
		args = {
				...
				name = function()
					if ns.IS_DEV_MODE then
						return "|cff00ff00"
							.. L["Dev Mode: ACTIVE"]
							.. "|r\n\n"
							.. L["The Discovery Tool is automatically tracking currencies and quests in the background."]
					else
						return "|cffff0000"
							.. L["Dev Mode: INACTIVE"]
							.. "|r\n\n"
							.. L["Dev tools are only available when running from a git clone (DevMarker.lua present)."]
					end
				end,
				fontSize = "medium",
				order = 1,
			...
			discoveryHeader = {
				type = "header",
				name = L["Discovery Tool"],
				order = 10,
				hidden = function()
					return not ns.IS_DEV_MODE
				end,
			},
			discoveryDesc = {
				type = "description",
				name = L["Discovers new currencies and quests to add to Weekly's database. Items are tracked automatically - the window just displays what's been found."],
				order = 11,
				hidden = function()
					return not ns.IS_DEV_MODE
				end,
			...
			discoveryStats = {
				...
				name = function()
					...
					if WeeklyDB and WeeklyDB.dev and WeeklyDB.dev.discovery then
						...
						if lastSaved then
							savedInfo = "\n" .. L["Last saved: %s"]:format(lastSaved)
						end
					end

					return L["Total logged: %d items (%d currencies, %d quests)%s"]:format(
						total,
						currencyCount,
						questCount,
						savedInfo
					) .. "\n|cff888888" .. L["Storage cap: 500 items (oldest auto-pruned)"]
				end,
				fontSize = "medium",
				order = 12,
				hidden = function()
			...
			openDiscovery = {
				type = "execute",
				name = L["Open Discovery Window"],
				desc = L["Open the Discovery Tool window to view and export logged items."],
				order = 15,
				func = function()
			...
			clearDiscovery = {
				type = "execute",
				name = L["Clear Logged Items"],
				desc = L["Clear all items logged this session."],
				order = 16,
				func = function()
					if ns.Discovery and ns.Discovery.tracker then
						ns.Discovery.tracker:Clear()
						print("|cff00ff00[Weekly Discovery]|r " .. L["Clear Logged Items"])
					end
				end,
				confirm = true,
				confirmText = L["Clear all items logged this session?"],
				hidden = function()
					return not ns.IS_DEV_MODE
				end,
			},
	...
	AceConfigDialog:AddToBlizOptions("Weekly_Dev", L["Dev Tools"], "Weekly")
end
